...
+    # Normalize both vectors to ensure scale invariance and handle zero vectors  # ✅ Fixed
+    a = np.asarray(a)  # ✅ Fixed
+    b = np.asarray(b)  # ✅ Fixed
+    a_norm_val = np.linalg.norm(a)  # ✅ Fixed
+    b_norm_val = np.linalg.norm(b)  # ✅ Fixed
+    if a_norm_val == 0 or b_norm_val == 0:  # ✅ Fixed
+        return 0.0  # ✅ Fixed
+    a_norm = a / a_norm_val  # ✅ Fixed
+    b_norm = b / b_norm_val  # ✅ Fixed
...
+    if not doc_embeddings or top_k <= 0:  # ✅ Fixed
+        return []  # ✅ Fixed
+    similarities = [cosine_sim(query_emb, doc_emb) for doc_emb in doc_embeddings]  # ✅ Fixed
+    # Sort in descending order of similarity  # ✅ Fixed
...
+    k = min(top_k, len(ranked_indices))  # ✅ Fixed
...
+        return chunks  # ✅ Fixed
+  # ✅ Fixed
+    # Ensure progress even if overlap is large  # ✅ Fixed
...
+    best_f1 = -1.0  # ✅ Fixed
+  # ✅ Fixed
+    if not similarities or not relevance or len(similarities) != len(relevance):  # ✅ Fixed
+        return best_threshold  # ✅ Fixed
+  # ✅ Fixed
+    for threshold in np.arange(0.0, 1.0001, 0.1):  # ✅ Fixed
...
+  # ✅ Fixed
+        # Correct F1 score formula  # ✅ Fixed
