...
def chunk_document(text: str, chunk_size: int = 100, overlap: int = 20) -> List[str]:
    ...
    if step <= 0:
        # Ensure progress and at least some overlap; if overlap >= chunk_size, fall back to step=1
        step = 1
...
def compute_embedding_quality(embeddings: List[np.ndarray]) -> Dict[str, float]:
    ...
    emb_list = [np.asarray(emb, dtype=np.float64) for emb in embeddings]
    norms = [np.linalg.norm(emb) for emb in emb_list]
    # Variance over all embedding values to reflect dispersion even if norms are equal
    flat_vals = np.concatenate([emb.ravel() for emb in emb_list]) if emb_list else np.array([], dtype=np.float64)
