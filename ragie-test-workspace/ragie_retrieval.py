...
def cosine_sim(a: np.ndarray, b: np.ndarray) -> float:
    ...
    # Handle zero-vector cases to avoid division by zero and undefined cosine.
    ...
    # Normalize b as well (fix): cosine similarity requires both vectors normalized
    b_norm = b / b_norm_val
...
def rank(documents: List[str], query_emb: np.ndarray, doc_embeddings: List[np.ndarray], top_k: int = 5) -> List[int]:
    ...
    # Sort in descending order of similarity (higher is better)
    ranked_indices = sorted(range(len(similarities)), key=lambda i: similarities[i], reverse=True)
    # Determine how many items to return
    k = len(ranked_indices) if top_k is None or top_k <= 0 else min(top_k, len(ranked_indices))
